...
angular.module('zeus.controllers')
  .controller('AppCtrl', function (ENV, $scope, $log, $timeout, $rootScope, $ionicPopup, $ionicLoading, Messages, Tabs, User, Settings, Push) {
    $log.log('app ctrl');
    ...
    $rootScope.getMessageCount = function () {
      Messages.getMessageCount().$promise.then(function(response){
        ...
        setBadge($scope.messageCount);
      },function(response){
        $log.log('get message count fail', response);
      });
    ...
    //set badge of app icon//徽章
    var setBadge = function (num) {
      // Promot permission request to show badge notifications
      if (window.cordova && windows.cordova.plugins.notification.badge) {
        cordova.plugins.notification.badge.hasPermission(function (granted) {
          $log.log('Permission has been granted:' + granted);
          if (granted) {
    ...
    $rootScope.$on('messagesMarkedAsRead',function(){
      ...
      setBadge($scope.messageCount);
      //reset badge
      Push.setBadge($scope.messageCount);
    });
    ...
    $scope.onHoldLogin = function(){
      ...
      if(window.cordova && window.cordova.plugins.clipboard){
        cordova.plugins.clipboard.paste(function(text){
          ...
          if(text){
            $log.log('get Accesss Token',text);
            $ionicLoading.show();
    ...
    $scope.login = function(){
      ...
      if(window.cordova && window.cordova.plugins.barcodeScanner){
        var loginPrompt = $ionicPopup.show({
          template:'PC端登陆cnodejs.org后，扫描设置页面的Access Token二维码可完成登录',
          title:'扫码登录',
          scope:$scope,
          buttons:[
            {
        ...
        if(ENV.debug){
          ...
          var loginPopup = $ionicPopup.show({
            ...
            subTitle:'PC端登陆cnodejs.org后，在设置页可以找到Access Token',
            scope:$scope,
            button:[
              {text:'取消'},
              {
